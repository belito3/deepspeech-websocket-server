...
class Audio(object):
    ...
    def __init__(self, callback=None, buffer_s=0, flush_queue=True):
        ...
        if callback is None:
            callback = lambda in_data: self.buffer_queue.put(in_data, block=False)
...
class VADAudio(Audio):
    ...
    def vad_collector(self, padding_ms=300, ratio=0.75, blocks=None):
        ...
        """
        if blocks is None:
            blocks = self.block_generator()

        num_padding_blocks = padding_ms // self.block_duration_ms
        print("num_padding_blocks: ", num_padding_blocks)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--server', default='ws://localhost:8005/recognize',
                        help="Default: ws://localhost:8005/recognize")
